...
# 5b. The update re-ran sync-packs (copier _tasks): the derived ontology catalog is back, not stale.
[ -f "$INST/.claude/enabled-packs.json" ] || { echo "copier-update: ontology catalog NOT regenerated by update — sync-packs.sh missing from copier _tasks (issue #200)" >&2; exit 1; }
echo "  copier: ontology catalog regenerated on update (sync-packs in _tasks)"
