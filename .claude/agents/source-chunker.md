...
- **Exhaustive enumeration over frugality** — emit EVERY distinct high-confidence
  entity/concept/finding the document yields as its own item; do not consolidate or
  rank-and-cut to a curated top-N, and never drop a real member as "minor." There is no
  finding-count cap — the harness keeps the complete set and the report-synthesizer
  decides what to foreground. Under-capturing is the expensive failure.
